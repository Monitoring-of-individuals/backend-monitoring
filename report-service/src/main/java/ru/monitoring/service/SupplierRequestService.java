...
public class SupplierRequestService {
    ...
    public ApiCloudBalance getApiCloudBalance() {
        ...
        return JSON.isValid(response) ? JSON.parseObject(response, ApiCloudBalance.class)
                : new ApiCloudBalance();
    }
    ...
    private FsspResponse getEnfProcessingsCheck(PersonInfoRequestDto personInfo) {
        MultiValueMap paramMap = new LinkedMultiValueMap();
        ...
        return JSON.isValid(response) ? JSON.parseObject(response, FsspResponse.class)
                : new FsspResponse();
    }

    private InnResponse getInnCheck(PersonInfoRequestDto personInfo) {
        String seriesNumber = personInfo.getPassport()
                .trim();
        MultiValueMap paramMap = new LinkedMultiValueMap();
        ...
        return JSON.isValid(response) ? JSON.parseObject(response, InnResponse.class)
                : new InnResponse();
    }
    ...
    private SelfEmplResponse getSelfEmplCheck(InnResponse inn) {
        ...
        log.info("Request a report to FNS by parameters: {}", paramMap);
        String response = apiCloudClient.getApiCloudResponse("/nalog.php", paramMap);
        return JSON.isValid(response) ? JSON.parseObject(response, SelfEmplResponse.class)
                : new SelfEmplResponse();
    }

    private PassportCheckResponse getPassportCheck(PersonInfoRequestDto personInfo) {
        String series = personInfo.getPassport()
                .substring(0, 4);
        String number = personInfo.getPassport()
                .substring(4);
        ...
        return JSON.isValid(response) ? JSON.parseObject(response, PassportCheckResponse.class)
                : new PassportCheckResponse();
    }

    private GibddResponse getDriverIdCheck(PersonInfoRequestDto personInfo) {
        if (personInfo.getDrivingLicence() == null || personInfo.getDateOfLicence() == null) {
            GibddResponse gibddResponse = new GibddResponse();
            gibddResponse.setMessage(
                    "В запросе не были предоставлены полные необходимые данные по водительскому удостоверению");
        ...
        return JSON.isValid(response) ? JSON.parseObject(response, GibddResponse.class)
                : new GibddResponse();
    }

    private RosFinMonResponse getTerrorExtrCheck(PersonInfoRequestDto personInfo) {
        String name = personInfo.getFatherName() != null ? personInfo.getLastName()
                .toUpperCase() + " " + personInfo.getFirstName()
                .toUpperCase() + " " + personInfo.getFatherName()
                .toUpperCase() : personInfo.getLastName()
                .toUpperCase() + " " + personInfo.getFirstName()
                .toUpperCase();
        ...
        RosFinMonResponse rosFinMonResponse = JSON.isValid(response) ? JSON.parseObject(response,
                RosFinMonResponse.class) : new RosFinMonResponse();

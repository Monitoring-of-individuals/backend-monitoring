...
public class ApiCloudClient {
     ...
     */
    public String getApiCloudResponse(String service,
                                      MultiValueMap paramMap) {
        ...
        return client.get()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, error -> Mono.error(
                        new ApiCloudResponseException(error.statusCode() + " " + error.getClass()
                                .toString())))
                .onStatus(HttpStatusCode::is5xxServerError, error -> Mono.error(
                        new ApiCloudResponseException(error.statusCode() + " " + error.getClass()
                                .toString())))
                .bodyToMono(String.class)
                .doOnError(error -> {
                    log.error("Произошла ошибка: {}, подробности: {}, стэктрейс: {}",
                            error.getClass(), error.getMessage(), error.getStackTrace());
                })
